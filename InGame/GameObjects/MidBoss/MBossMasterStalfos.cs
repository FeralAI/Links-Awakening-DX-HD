...
class MBossMasterStalfos : GameObject
{
    ...
    private readonly SheetAnimator _animator;

    private readonly CPosition[] _positions = new CPosition[5];
    private readonly CSprite[] _sprites = new CSprite[5];
    private readonly CSprite _sprite;

    // quick fix: draw the swing behind the sword
    private readonly int[] _drawOrder = { 0, 1, 2, 4, 3 };

    private const float MovementSpeed = 0.5f;

    private readonly float[] DamageTime = { 0, 175, 75, 0 };
    private readonly float[] _damageCounters = new float[4];
    private readonly float[] _partVelocity = new float[4];
    private readonly float[] _partOffset = { 0, 0, 3, 3 };
    private readonly float _partGravity = 0.15f;

    private readonly float[] _standCounters = new float[4];
