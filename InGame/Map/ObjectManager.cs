...
public class ObjectManager(Map owner)
{
    // TODO_End check if everything is cleaned while loading a new map
    public Map Owner = owner;

    public List<GameObjectItem> ObjectList = [];
    private readonly List<GameObject> SpawnObjects = [];
    public List<GameObject> DeleteObjects = [];

    public Texture2D ShadowTexture;
    public static Effect CurrentEffect;

    private readonly List<GameObject> _poolSpawnedObjects = [];

    private ComponentPool _gameObjectPool;
    private ComponentDrawPoolNew _drawPool;

    private readonly SystemBody _systemBody = new();
    private readonly SystemAi _systemAi = new();
    private readonly SystemAnimation _systemAnimator = new();

    private readonly List<KeyChangeListenerComponent.KeyChangeTemplate> _keyChangeListeners =
        [];

    // lists are used to not generate new ones every time
    // one list object would probably be enough?
    private readonly List<GameObject> _updateGameObject = [];
    private readonly List<GameObject> _damageFieldObjects = [];
    private readonly List<GameObject> _drawShadowObjects = [];
    private readonly List<GameObject> _depthObjectList = [];
    private readonly List<GameObject> _objectTypeList = [];
    private readonly List<GameObject> _objectTagAllList = [];
    private readonly List<GameObject> _collisionObjectList = [];

    private readonly List<GameObject> _collidingObjectList = [];
    private readonly List<GameObject> _lightObjectList = [];
    private readonly List<GameObject> _carriableObjectList = [];
    private readonly List<GameObject> _hittableObjectList = [];
    private readonly List<GameObject> _pushableObjectList = [];
    private readonly List<GameObject> _interactableObjectList = [];

    private readonly List<GameObject> db_hittableList = [];
    private readonly List<GameObject> db_damageList = [];
    private readonly List<GameObject> db_bodyList = [];
    private readonly List<GameObject> db_gameObjectList = [];
