...
class BossFinalBoss : GameObject
{
    ...
    private readonly Vector2[] _fireballOffset = new Vector2[] { new(-15, -8), new(0, -24), new(15, -8), new(0, -4) };

    private readonly Vector2[] _bodyParts = new Vector2[6];
    ...
    private readonly BossFinalBossTail[] _moldormTails = new BossFinalBossTail[4];
    private readonly AiTriggerSwitch _moldormSpeedUp;
    private readonly Vector2[] _moldormPositions = new Vector2[6 * TailMult];
    private readonly Vector2[] _moldormPositionsNew = new Vector2[6 * TailMult];
    ...
    private readonly int _moveDist = 40;
    ...
    private readonly float[] _finalPartDistance = new float[] { 18, 10, 10, 10 };
    ...
    private readonly int _finalStateDeathCounter = 2500;
    ...
    private void UpdateMoldormTail(CPosition position)
    {
        ...
        {
            for (var i = 0; i < _partDist.Length; i++)
            {
                if (_partDist[i] > 0)
                {
        ...
        for (var i = 1; i < _moldormPositions.Length; i++)
        {
