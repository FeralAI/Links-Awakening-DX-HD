...
public partial class ObjLink : GameObject
{
    ...
    private readonly List<GameObject> _ocarinaList = [];
    private readonly List<GameObject> _destroyableWallList = [];
    ...
    private readonly Vector2[] _walkDirection = { new(-1, 0), new(0, -1), new(1, 0), new(0, 1) };
    ...
    private readonly int _animationOffsetX = -7;
    private readonly int _animationOffsetY = -16;
    ...
    // weapon animations
    private readonly Animator AnimatorWeapons;
    ...
    public RectangleF PlayerRectangle => new(PosX - 4, PosY - 12 - PosZ, 8, 12);
    private readonly BodyDrawComponent _drawBody;
    private readonly BodyDrawShadowComponent _shadowComponent;
    private readonly DrawComponent.DrawTemplate _bodyDrawFunction;
    ...
    private readonly Vector2[] _showInstrumentOffset = {
        new(-dist1, -dist0), new(dist1, -dist0), new(dist0, dist1), new(dist0, -dist1),
        new(dist1, dist0),new(-dist1, dist0),new(-dist0, -dist1),new(-dist0, dist1) };
    ...
    private readonly float _bootsRunTime = 500;
    private readonly float _bootsParticleTime = 120;
    ...
    private readonly float _swordPokeTime = 100;
    private float _swordPokeCounter;

    private readonly Vector2[] _shootSwordOffset;
    ...
    private readonly Point[] _pokeAnimationOffset;
    private bool _isHoldingSword;
    private bool _isSwingingSword;
    public bool CarrySword;

    // items
    private readonly ObjBoomerang _boomerang = new();
    private readonly Vector2[] _boomerangOffset;
    private readonly Vector2[] _arrowOffset;
    ...
    private readonly Vector2[] _hookshotOffset;
    private bool _hookshotPull;

    // magic rod
    private readonly Vector2[] _magicRodOffset;
    private const float MagicRodSpeed = 3;
    private const float MagicRodSpeedPoP = 4;

    // shovel
    private readonly Vector2[] _shovelOffset;
    private Point _digPosition;
    private bool _hasDug;
    private bool _canDig;

    private readonly Vector2[] _powderOffset;
    private readonly Vector2[] _bombOffset;
    ...
    // cloak transition
    private readonly int CloakTransitionTime = 2200;
    private float _cloakTransitionCounter;
    private float _cloakPercentage;
    private readonly int CloakTransitionOutTime = 2500;
    ...
    // instrument stuff
    // @TODO: replace
    private readonly Rectangle[] _noteSourceRectangles = { new(145, 97, 10, 12), new(156, 97, 6, 12) };
    private readonly bool[] _noteInit = { false, false };
    private readonly int[] _noteSpriteIndex = { 0, 0 };
    ...
    private readonly DictAtlasEntry _stunnedParticleSprite;
