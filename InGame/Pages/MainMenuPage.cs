...
class MainMenuPage : InterfacePage
{
    ...
    private readonly float[] _playerSelectionState = new float[SaveStateManager.SaveCount];
    private readonly InterfaceImage[] _playerImage = new InterfaceImage[SaveStateManager.SaveCount];
    private readonly InterfaceButton[] _saveButtons = new InterfaceButton[SaveStateManager.SaveCount];

    private readonly Dictionary<string, object> _newGameIntent = [];

    private readonly Animator _playerAnimation = new();
    private readonly DictAtlasEntry _heartSprite;

    private readonly InterfaceElement[][] _heartImage = new InterfaceElement[4][];

    private readonly InterfaceGravityLayout[] _saveButtonLayouts = new InterfaceGravityLayout[SaveStateManager.SaveCount];
    private readonly InterfaceLabel[] _saveNames = new InterfaceLabel[SaveStateManager.SaveCount];
    private readonly InterfaceLabel[] _saveRuby = new InterfaceLabel[SaveStateManager.SaveCount];
    private readonly InterfaceListLayout[] _deleteCopyLayouts = new InterfaceListLayout[SaveStateManager.SaveCount];

    private readonly InterfaceListLayout _mainLayout;
    private readonly InterfaceListLayout _newGameButtonLayout;

    private readonly InterfaceListLayout _menuBottomBar;
    private readonly InterfaceListLayout _saveFileList;
