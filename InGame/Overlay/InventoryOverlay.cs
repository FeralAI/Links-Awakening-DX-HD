...
class InventoryOverlay
{
    ...
    private readonly Rectangle _background0 = new(0, 0, 268, 24);
    private readonly Rectangle _background1 = new(0, 26, 268, 182);

    private readonly Rectangle _keyRectangle = new(94, 6, 20, 114);
    private readonly Rectangle[] _keyPositions = new Rectangle[5];

    private readonly Rectangle _relictsRectangle = new(118, 6, 144, 22);
    private readonly Point _relictPosition = new(123, 9);
    ...
    private readonly Point _heartsPosition = new(6, 5);
    ...
    private readonly Rectangle _tradeStuffRectangle = new(6, 6, 84, 22);
    ...
    private readonly Point _itemSlotsPosition = new(14, 41);

    private readonly Rectangle _skirtRectangle = new(180, 31, 16, 15);
    private readonly Rectangle _skirtColorRectangle = new(198, 37, 14, 10);
    private readonly Rectangle _heartPiecesRectangle = new(4, 72, 16, 14);

    private const int ItemSlotWidth = 4;

    public static Rectangle RecItemselection = new(0, 0, 30, 20);

    public const int DistX = 8;
    public const int DistY = 5;

    private static readonly string[] _itemSlotString = new[] { "A", "B", "X", "Y" };

    //  3
    // 2 1
    //  0
    private static readonly Rectangle[] _itemSlots = {
        new(RecItemselection.Width + DistX / 2 - RecItemselection.Width / 2,
            RecItemselection.Height * 2 + DistY * 2,
            RecItemselection.Width, RecItemselection.Height),
        new(RecItemselection.Width + DistX, RecItemselection.Height + DistY,
            RecItemselection.Width, RecItemselection.Height),
        new(0, RecItemselection.Height + DistY,
            RecItemselection.Width, RecItemselection.Height),
        new(RecItemselection.Width + DistX / 2 - RecItemselection.Width / 2, 0,
            RecItemselection.Width, RecItemselection.Height)
    };
    ...
    private readonly Point _itemRectangleSize = new(27, 26);
    private readonly Point _itemRecMargin = new(0, 0);

    private readonly Point _equipmentPosition = new(6, 123);
    private readonly Rectangle _itemsRectangle = new(6, 124, 108, 52);
