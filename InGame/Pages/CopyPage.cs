...
{
    private readonly InterfaceListLayout[] _fromSaveSlot = new InterfaceListLayout[SaveStateManager.SaveCount];
    private readonly InterfaceLabel[] _saveFromNames = new InterfaceLabel[SaveStateManager.SaveCount];

    private readonly InterfaceListLayout[] _toSaveSlot = new InterfaceListLayout[SaveStateManager.SaveCount];
    private readonly InterfaceLabel[] _saveToNames = new InterfaceLabel[SaveStateManager.SaveCount];
    ...
    public override void OnReturn(Dictionary<string, object> intent)
    {
        ...
        if (intent != null && intent.TryGetValue("copyFile", out var copyFile) && (bool)copyFile)
        {
            ...
            {
                { "copyTargetSlot", _targetSlotIndex }
            };
