...
class TileExtractor(string screenId) : Screen(screenId)
{
    private List<Texture2D> _sprTiled = [];
    private List<Texture2D> _sprTiledRemoved = [];

    private readonly List<Rectangle> _untiledParts = [];
    ...
    private Point _tileSize = new(16, 16);
    ...
    private readonly int _toolBarWidth = 200;
    ...
    private Texture2D RenderTileTexture(GraphicsDevice graphicDevice, List<Texture2D> tiledInput, int tileWidth, int tileHeight, Point tilePadding)
    {
        try
        {
            ...
            for (var y = 0; y < newTextureHeight; y++)
                for (var x = 0; x < newTextureWidth; x++)
                {
